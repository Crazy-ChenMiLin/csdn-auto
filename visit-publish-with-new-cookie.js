...
async function visitPublishWithNewCookie() {
  ...
  try {
    console.log(`${currentTime} - 使用新Cookie访问小红书发布页面 ${publishUrl}`);
    ...
    await page.screenshot({ 
      path: `publish-page-initial-${currentTime.replace(/[:.]/g, '-')}.png`,
      fullPage: true
    });
...
// 立即执行
visitPublishWithNewCookie().catch(error => console.error('访问失败:', error));
