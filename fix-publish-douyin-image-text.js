...
async function fixPublishDouyinImageText() {
  ...
  try {
    console.log(`${currentTime} - 开始修复后的图文发布测试`);
    ...
    // 保存初始页面截图
    const initialScreenshot = `douyin-fix-initial-${currentTime.replace(/[:.]/g, '-')}.png`;
    ...
    if (uploadElement) {
      ...
      if (tagName === 'input' && await uploadElement.evaluate(el => el.type === 'file')) {
        ...
        if (fs.existsSync(imagePath)) {
          ...
          const uploadScreenshot = `douyin-fix-uploaded-${currentTime.replace(/[:.]/g, '-')}.png`;
    ...
    if (publishButton) {
      ...
      if (isEnabled) {
        ...
        const publishScreenshot = `douyin-fix-published-${currentTime.replace(/[:.]/g, '-')}.png`;
    ...
    // 保存最终状态截图
    const finalScreenshot = `douyin-fix-final-${currentTime.replace(/[:.]/g, '-')}.png`;
    await page.screenshot({ path: finalScreenshot, fullPage: true });
    console.log(`${currentTime} - 最终页面截图已保存: ${finalScreenshot}`);

    await browser.close();
    console.log(`${currentTime} - 修复后的图文发布测试完成`);

  } catch (error) {
    console.error(`${currentTime} - 任务失败: ${error.message}`);
    if (page) {
      const errorScreenshot = `douyin-fix-error-${currentTime.replace(/[:.]/g, '-')}.png`;
      try {
...
fixPublishDouyinImageText().catch(error => console.error('修复后的图文发布测试失败:', error));
