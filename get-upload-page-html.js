...
async function getUploadPageHtml() {
  ...
  try {
    console.log(`${currentTime} - 开始获取上传页面HTML内容`);
    ...
    const htmlPath = `upload-page-html-${currentTime.replace(/[:.]/g, '-')}.html`;
    fs.writeFileSync(htmlPath, htmlContent);
    console.log(`${currentTime} - 页面HTML已保存到: ${htmlPath}`);

    // 获取可见文本
    const visibleText = await page.textContent('body');
    const textPath = `upload-page-text-${currentTime.replace(/[:.]/g, '-')}.txt`;
    fs.writeFileSync(textPath, visibleText);
    console.log(`${currentTime} - 页面可见文本已保存到: ${textPath}`);

    // 检查页面上的关键元素
    const fileInputs = await page.$$('input[type="file"]');
    console.log(`${currentTime} - 文件输入框数量: ${fileInputs.length}`);
    ...
    console.log(`${currentTime} - 文本输入框数量: ${textInputs.length}`);

    // 保存截图
    const screenshotPath = `upload-page-html-screenshot-${currentTime.replace(/[:.]/g, '-')}.png`;
    await page.screenshot({ path: screenshotPath, fullPage: true });
    console.log(`${currentTime} - 页面截图已保存到: ${screenshotPath}`);

    await browser.close();
    console.log(`${currentTime} - 获取完成`);
...
getUploadPageHtml().catch(error => console.error('获取上传页面HTML失败:', error));
