...
async function visitXiaohongshuPublish() {
  ...
  try {
    ...
    fs.writeFileSync(`xiaohongshu-publish-content-${currentTime.replace(/[:.]/g, '-')}.html`, pageContent);
    console.log(`${currentTime} - 页面HTML内容已保存`);

    // 检查是否有文件选择器元素 - 尝试不同的选择器
    ...
    for (const selector of possibleSelectors) {
      try {
        ...
        if (element) {
          console.log(`${currentTime} - 文件选择器元素找到，使用选择器: ${selector}`);
          fileInputFound = true;
